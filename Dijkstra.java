...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
        ...
        double eucDist = Math.sqrt(((ux - vx) * (ux - vx)) + ((uy - vy) * (uy - vy)));
        return eucDist;
  }
  ...
  public void computeAllEuclideanDistances() {
        ...
        for(String string: vertexNames.keySet()){

          for(Edge edge: vertexNames.get(string).adjacentEdges){

            edge.distance = computeEuclideanDistance(vertexNames.get(string).x, vertexNames.get(string).y, edge.target.x, edge.target.y);
  ...
  public void doDijkstra(String s) {
		...
		for(String string: vertexNames.keySet()){
			...
			vertexNames.get(string).prev = null;
			vertexNames.get(string).distance = Double.MAX_VALUE;
			vertexNames.get(string).known = false;
			list.add(string);
		...
		while(list.isEmpty() == false){
			...
			for(String string: list){
				...
				if(vertexNames.get(string).distance < distance){

					distance = vertexNames.get(string).distance;
			...
			list.remove(v);
			vertexNames.get(v).known = true;

			for(Edge edge: vertexNames.get(v).adjacentEdges){

				if(edge.target.known == false){

					double d = vertexNames.get(v).distance + edge.distance;

					if(d < edge.target.distance){

						edge.target.distance = d;
						edge.target.prev = vertexNames.get(v);
					}
				}
			}
		}
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    ...
    List<Edge> dPath = new LinkedList<Edge>();

	  Vertex beginning = vertexNames.get(s);
    ...
    while(beginning != end){
      ...
      if(previous == null){
        ...
        List<Edge> emp = new LinkedList<Edge>();
        return emp;
    ...
    /*Vertex d = vertexNames.get(t);
    Vertex prevN = d.prev;

    while(prevN){

      for(Edge edge: prevN.adjacentEdges){

        if(edge.target.name.equals(d.name)){
